...
def train_model(model,
    ...
    iter_update = 'train reconstruction loss: {1:.4e}, val reconstruction loss: {2:.4e}\r'
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            with tqdm(total=batches_per_eval, desc=' Eval') as pbar:
                for (xbt, ybt), (xbv, ybv) in zip(train_loader, val_loader):
                    ...
                    train_loss += (Loss.reconstruction_loss(ybt, train_reconstructions))


                    #compute val loss
                    val_segmentations, val_reconstructions = model(xbv)
                    val_loss += (Loss.reconstruction_loss(ybv, val_reconstructions))
        ...
        with tqdm(total=eval_interval, desc='Train') as pbar:
            for xb, yb in train_loader:
                ...
                if torch.isnan(l_reconstruction):
